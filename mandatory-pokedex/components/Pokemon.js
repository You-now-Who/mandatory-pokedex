...
const Pokemon = () => {
  ...
  const handlePageChange = (pageNumber) => {
    ...
    useEffect(() => {
      pokemonService.getAll(numPokemon).then((data) => {
    ...
    const filteredItems = currentItems.filter(
      (pokemon) =>
        ...
        pokemon.types.some((type) => type.type.name.includes(filterType)) &&
        pokemon.abilities.some((ability) =>
          ability.ability.name.includes(filterAbility)
        )
    );
    useEffect(() => {
        ...
        <table className="table-auto w-3/4 text-white font-mono text-lg">
          <thead className="bg-indigo-900 text-white">
            <tr>
              ...
              <th className="px-4 py-2 font-bold text-xl text-shadow">
                Height
              </th>
              <th className="px-4 py-2 font-bold text-xl text-shadow">
                Weight
            ...
            {currentItems.map((pokemon, index) => (
              <tr
                ...
                className={`text-center ${
                  index % 2 === 0 ? "bg-indigo-700" : "bg-indigo-800"
        ...
        <div className="flex justify-center mt-5 space-x-2">
          {pageNumbers.map((number) => (
            <button
              ...
              className={`px-3 py-2 mb-5 border border-indigo-500 rounded-full hover:bg-indigo-500 hover:text-white transition-colors duration-200 ${
                currentPage === number
